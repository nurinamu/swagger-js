...
SwaggerSpecConverter.prototype.toJsonSchema = function(source) {
  ...
  var detectedType = ( (source.parameterType && source.parameterType.absoluteType) || (source.type && source.type.absoluteType) || source.type || source.dataType || source.responseClass || '');
  ...
  if(lcType.indexOf('list[') === 0) {
    ...
    console.log('last');
    console.log(source);
    return {$ref: '#/definitions/' + this.modelMap[detectedType] || detectedType};
  }
};
